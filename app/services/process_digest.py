...
def _fallback_digest(title: str, content: str) -> DigestOutput:
    text = " ".join((content or "").split())
    if len(text) > 400:
        text = text[:397] + "..."
    if not text:
        text = title
    return DigestOutput(title=title[:120], summary=text)
...
def process_digests(limit: Optional[int] = None) -> dict:
    ...
    mode = "fallback (no OpenAI)" if USE_FALLBACK_DIGESTS else "OpenAI"
    logger.info(f"Starting digest processing for {total} articles using {mode}")
    ...
    for idx, article in enumerate(articles, 1):
        ...
        try:
            if USE_FALLBACK_DIGESTS:
                digest_result = _fallback_digest(article["title"], article["content"])
            else:
